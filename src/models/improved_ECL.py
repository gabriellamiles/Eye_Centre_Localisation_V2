...
if __name__ == '__main__':
    ...
    # obtain filepaths of all csv files containing bounding box and eye centre data
    root_folder = os.getcwd()
    label_folder = os.path.join(root_folder, "data", "processed", "combined_labels")
    # return list of all paths in directory, and add label_folder to the filepath as prefix
    label_filepaths = [os.path.join(label_folder, i) for i in os.listdir(label_folder)]

    img_folder = os.path.join(rootFolder, "data", "processed", "mnt", "eme2_square_imgs") # image folder temporarily mounted elsewhere so use below
    img_folder = img_folder.replace("Eye_Centre_Localisation_V2", "Eye_Region_Detection")

    ######## LOAD AND CREATE DATASET AND PREPROCESS DATA #####

    # load bounding box and eye centre data
    list_of_df = load_csv_files(label_filepaths)
    full_df = combine_list_into_single_df(list_of_df)

    # load corresponding images (of only bounding box region) - assign 0 to left eye (as viewed from screen), 1 for right eye


    # split dataset into train, validation, and test sets for respective folds
    ...
    # plot output from model training
